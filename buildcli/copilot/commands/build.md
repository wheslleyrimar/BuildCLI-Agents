...
2. Read `worklist.md`. Pick the next pending unit, respecting the blocked-by ordering.
3. Load the band skill for that unit from `.github/skills/<band>/SKILL.md`.
4. Read only the matching `[band:<band>]` block from `.buildcli/context.md`.
5. Implement the unit following the skill's workflow.
6. Mark the unit `done` in `worklist.md`.
7. Return: unit completed, files changed, flags for downstream units.
